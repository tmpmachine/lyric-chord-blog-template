let BlogFeeds = (function() {
  ...
  function BlogFeeds(config) {
    ...
    function getUpdateURL() {
      let URL = `https://www.blogger.com/feeds/${_.blogId}/posts/summary/-/search-hint?alt=json&max-results=${option.maxResults}&orderby=updated&updated-min=${settings.updated}&start-index=${(settings.start*1-1) * option.maxResults + 1}&rand=${(new Date()).getTime()}`;
      return URL;
    }
    ...
    function checkIntegrity() {
      let URL = `https://www.blogger.com/feeds/${_.blogId}/posts/summary/-/search-hint?alt=json&max-results=0&orderby=updated&rand=${(new Date()).getTime()}`;
      const request = new XMLHttpRequest();
